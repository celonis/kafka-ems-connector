...
import com.celonis.kafka.connect.ems.storage.InMemoryFileSystem
...
class InMemoryParquetFormatWriterTests extends AnyFunSuite with Matchers with WorkingDirectory with SampleData {
  test("rollover on schema change returns true") {
    withDir { dir =>
      val sinkName       = "sA"
      val topicPartition = TopicPartition(new Topic("B"), new Partition(2))
      val output         = InMemoryFileSystem.createOutput(dir, sinkName, topicPartition)
      output.size shouldBe 0L
      val formatWriter =
        InMemoryParquetFormatWriter.from(output, simpleSchemaV1, ParquetConfig.Default, new NoOpExploder().explode)
  ...
  test("writes a schemaless JSON") {
    withDir { dir =>
      ...
      val formatWriter =
        InMemoryParquetFormatWriter.from(output, struct.getSchema, ParquetConfig.Default, new NoOpExploder().explode)
      formatWriter.write(struct)
      formatWriter.close()
      formatWriter.size > 4 shouldBe true
    }
  ...
  test("flush to disk after N records") {
    withDir { dir =>
      ...
      val count = 100
      val formatWriter = InMemoryParquetFormatWriter.from(output,
                                                          simpleSchemaV1,
