...
case class UploadRequest(output: Output, topic: Topic, partition: Partition, offset: Offset)
object UploadRequest {
  implicit val show: Show[UploadRequest] =
    Show.show(r =>
      s"file=${r.output.show} topic=${r.topic.show} partition==${r.partition.show} offset=${r.offset.show}",
    )
}
trait Uploader[F[_]] {
