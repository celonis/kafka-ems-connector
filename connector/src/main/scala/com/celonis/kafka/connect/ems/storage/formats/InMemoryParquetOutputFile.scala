...
class InMemoryParquetOutputFile(out: InMemoryFile) extends OutputFile {
  override def create(blockSizeHint: Long): PositionOutputStream =
    new InPositionOutputStreamWrapper(out)
  ...
  class InPositionOutputStreamWrapper(fs: InMemoryFile) extends PositionOutputStream {
