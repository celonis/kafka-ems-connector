...
import com.celonis.kafka.connect.ems.storage.InMemoryFile
...
class InMemoryParquetFormatWriter(
  output:    InMemoryFile,
...
object InMemoryParquetFormatWriter {
  def from(
    output:    InMemoryFile,
    schema:    Schema,
    config:    ParquetConfig,
    explodeFn: GenericRecord => NonEmptySeq[GenericRecord],
  ): InMemoryParquetFormatWriter = {
    val outputFile = new InMemoryParquetOutputFile(output)
    ...
    new InMemoryParquetFormatWriter(output, writer, explodeFn)
  }
}
